...
const Productos = () => {
  ...
  const buscar = async () => {
    try {
      const response = await axios.get(
        `https://bazar-7e11.onrender.com/api/items/q?filter=${searchTerm}`
      );
  ...
  return (
    <div className="productosBuscados">
      ...
      {products.length > 0 ? (
        <>
          <h2>Se han encontrado {products.length} productos</h2>

          <div className="cardPersonalizada">
            {products.map((product) => (
              <Link key={product.id} to={`/item/${product.id}`}>
                <div key={product.id} className="cardPresentacion">
                  ...
                  <div className="detalles">
                    <div className="producto">
                      ...
                      <h4 className="titulo">{product.category}</h4>
                    </div>
